...
* Base64上传（完成）：客户端截屏等小文件上传使用（由于对不完整的部分base64字符串解码可能失败，httpServer层面自行进行聚合后再对完整信息进行base64解码，这是base64上传只适用于小文件上传的原因）。
* 断点上传（完成）
* Thunked下载（完成）
* 压力测试（完成）
* 指定文件删除（完成）
* 过期文件自动清理（暂不考虑实现）：思路为根据file-config.xml里存储空间的配置采用定期轮巡的方式进行清理（也可以自行集成其他定时任务框架）；
* 异常恢复（暂不考虑实现）：服务异常重启，断电宕机等情况下数据恢复（主要是未落盘的临时文件落盘，不过由于采用了零拷贝+顺序写的机制，临时文件落盘非常高效，一般情况很难撞上）；
